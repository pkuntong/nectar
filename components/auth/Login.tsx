...
const Login: React.FC<LoginProps> = ({ onLoginSuccess, onError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (data?.user) {
          // Non-blocking welcome email send (if backend email is configured)
          try {
            await convexClient.functions.invoke('send-email', {
              body: {
                to: email,
                subject: 'Welcome to Nectar Forge',
                html: '<p>Your account has been created successfully.</p><p>You can log in now.</p>',
                type: 'welcome',
              },
            });
          } catch {
            // Ignore email send failures; account creation should still succeed.
          }
  ...
  const handleGoogleSignIn = () => {
    onError('Google sign-in is currently disabled. Use email/password for now.');
  };
  ...
  return (
    <div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 bg-white/80 text-gray-700 font-semibold py-4 px-6 rounded-xl border border-gray-300 cursor-not-allowed"
      >
      ...
      <p className="text-xs text-center text-medium-text mt-4">
        Google sign-in will be re-enabled after provider setup is completed.
      </p>
    </div>
  );
};
